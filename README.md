...
The player is invited to guess which capital city I am hiding; if they are incorrect, I will tell them how far and in which direction they need to go, to find me.

This project was built as part of the Full Stack Software Development course with Code Institute.

[The game is available to play in a simulated terminal.](https://denham-adventure-capital.herokuapp.com/)
## User Stories

### Game Designer
As a developer and designer for this game, I wanted to create a simple, intuitive yet enjoyable game.
The game should be immediately understandable but provide enough feedback to keep players returning to beat their previous scores and to challenge others.
...
Simple design (colour, spacing and timing) should be used to provide feedback and prompt the user for information, as well as create a natural 'flow' to the game.
...
The first outer loop encourages the user to remain engaged and play consecutive games. If a player decides to play again, after a successful game, they do not need to reenter their name or express a hint preference.

The inner loop represents the guess and response dialogue within the game itself - the user(player) continues to guess until they successfully locate the opponent's (hider's) city.
### Class Design
...
When the user is prompted for a yes/no answer (shown as 'y'/'n') they will be unable to proceed until they have given the required answer. It is not case-sensitive.
...
This program uses a custom function that defines a style by keyword (eg. intro, warning etc) and prints the text to the user. This helps keep the text styling consistent by assigning a clear meaning to each type.
...
The underlying mathematical principles are fascinating, however, and well worth further exploration. A good starting place is [here.](https://www.geeksforgeeks.org/haversine-formula-to-find-distance-between-two-points-on-a-sphere/)
...
### Difficulty Settings
After feedback from testing, future versions will offer the user an opportunity to vary the difficulty of the game by choosing whether the mystery location will be a well-known capital or somewhere more obscure. 

However, there is one problem to overcome before implementation. "Well-known" is a relative term and, as the developer is from Europe, likely to mean that the easy mode is more euro-centric. This may be more difficult for players from other continents. As one of the objectives of the game is to help users better their knowledge of capital cities, it might be considered self-defeating to omit the lesser-known (again a relative term!) capitals of the world.

### Guess by Continent
Players will be able to select a continent at the start of the game; the chosen capital will then be limited to this continent, allowing users to focus their learning and gameplay on particular areas of the world.

The function that prepares the game (prepare_game) has already been separated from the main gameplay to allow this preparatory stage to get more complex and responsive to the user.
...
|---|---|---|
| User starts the programme by pressing Run Program | Welcome Title and Image are printed to the page. User is asked whether they want to see the instructions | PASS |

**Prompt for Instructions**
| **User Action** | **Expected Result** | **Pass / Fail** |
|---|---|---|
| User presses 'y' or 'Y' and Enter | Instructions are printed and user is prompted for name | PASS |
| User presses 'n' or 'N' and Enter | Message of acknowledgement printed and the user is prompted for name  | PASS |
| User presses any other key/keys and presses Enter | User is warned that input is invalid and prompted again | PASS |

**Prompt for Username**
| **User Action** | **Expected Result** | **Pass / Fail** |
|---|---|---|
| User enters a username comprised of alpha-numeric characters  | Username is acknowledged and the user is asked whether they would like hints | PASS |
| User enters a username with non-alpha-numeric characters | User is warned that input is invalid and prompted again | FAIL - Programme currently accepts punctuation marks and keys such as Esc |
...
| **User Action** | **Expected Result** | **Pass / Fail** |
|---|---|---|
| User presses 'y' or 'Y' and Enter | Message of acknowledgement printed and the game begins with an invite for a first guess | PASS |
| User presses 'n' or 'N' and Enter | Message of acknowledgement printed and the game begins with an invite for a first guess| PASS |
| User presses any other key/keys and presses Enter | User is warned that input is invalid and prompted again | PASS |

**Prompt for Guess**
| **User Action** | **Expected Result** | **Pass / Fail** |
|---|---|---|
| User enters a capital city in Title case (eg. London) | Confirmation message is displayed, depending on whether the guess is correct or not (see below) | PASS |
| User enters a capital city in Lower case (eg. budapest) | Confirmation message is displayed, depending on whether the guess is correct or not (see below) | PASS |
| User enters a capital city in Upper case (eg. ACCRA) | Confirmation message is displayed, depending on whether the guess is correct or not (see below) | PASS |
...
|  | The top 10 scores are displayed on the screen. | PASS |
...
Initially testing revealed that the player was able to guess a capital city by entering the name of the country. This was due to the GSpread search function searching the whole worksheet rather than a single column. This was corrected by specifying a value for the in_column parameter.
...
The problem was very hard to replicate but through manual testing (creating an infinite loop that requested the details of the same city until the requests were blocked by Google Sheets), the developer was able to simulate this error.
...
**Invalid Usernames**
 ...
 Testing revealed that users could enter a username that consisted only of blank spaces. This error would then repeat throughout the game and potentially be posted to the leaderboard.
...
2. All credentials for the compromised service account were revoked. The account was deleted.
3. The Google Sheet containing the data was duplicated and renamed, and the original was deleted.
...
3. Changes were committed using <code>git commit -m</code> with a clear, imperative message to describe the change
...
3. In the Settings tab, under Config Vars, set up the following environment variables:
    - CREDS with the value of the contents of creds.json
...
7. Enabled Automatic deploys - this means that every push to the main branch on GitHub will deploy a new version of this app. Alternatively, you can select a manual deployment.
...
[geographiclib](https://geographiclib.sourceforge.io/html/python/index.html#) is used, as described above, to solve the geodesic problems on an ellipsoid model of the earth. In this program, it is used to find the distance and azimuth between two sets of coordinates.

**From Python, the following modules are used:**

[random](https://docs.python.org/3/library/random.html) provides pseudo-random values. randint is used in this program to provide a random integer which acts as a row index from the list of capital cities. This allows the program to select a city for the player to guess.

[time](https://docs.python.org/3/library/time.html) is a module providing various time-related functions. Sleep is used in this program to delay the appearance of messages to the user; this is used purely for dramatic effect to build tension.

[uuid](https://docs.python.org/3/library/uuid.html) is used to provide a unique ID for each game. This allows the game to be identified in the list of all games, which is important for providing a percentile to the user after a successful game.
...
Scott Stafford's answer provides the basis for the function used to sort high scores (by the number of guesses and by distance).
...
This was used to identify the current game in the list of all games to provide a positional ranking as a percentage to the player.

[String Validators](https://www.hackerrank.com/challenges/string-validators/problem)

The explanations of the built-in string methods were extremely useful in fixing the validation of the username.
